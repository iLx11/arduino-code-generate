<template>
  <div id="esp32-list">
    ...
    <div id="component-box">
      ...
      <MqttConnect v-if="compActive.mqttComp.isActived" />
      <WebserverComp v-if="compActive.webserverComp.isActived" />
      <OledHardware v-if="compActive.oledComp.isActived" />
      <FileSystem v-if="compActive.fileComp.isActived" />
      <ApModel v-if="compActive.APComp.isActived" />
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  Object.keys(compActive).forEach((o) => (compActive[o].isActived = true))
})
// 替换模板代码
